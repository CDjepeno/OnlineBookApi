...
import { GetBookingUserPaginationResponse } from "@/types/booking/response.types";
...
export default function DashboardDetail({
  ...
  userBookings: GetBookingUserPaginationResponse;
  initalPage: number;
  totalPages: number;
}) {
  ...
  const { data: bookingPaginate } = useQuery({
    queryKey: [BookingsQueriesKeysEnum.GetBookingsUser, currentPage],
    queryFn: () => GetBookingsUser(user!.id, currentPage, totalPages),
    enabled: !!currentPage,
  });
  ...
  const rows =
    bookingsUser?.map((bookingUser) => ({
      cells: [
        ...
        <Stack direction="row" justifyContent="start" key={bookingUser.bookId}>
          <IconButton
            aria-label="edit"
